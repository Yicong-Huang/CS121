...
class TokenStore:
  ...
  def storeToken(self, token, page, prefix='token'):
  ...
  def getTokensOnPage(self, page):
    return (key for key, val in self.tokenOccurrencePairs() if str(page) in map(lambda x: x[0], val))

  def occurrences(self):
    return (pair[1] for pair in self.tokenOccurrencePairs())
