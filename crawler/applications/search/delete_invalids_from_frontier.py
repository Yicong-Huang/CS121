...
from datamodel.search.Yicongh1ZicanlHwo_datamodel import Yicongh1ZicanlHwoLink
...
@Deleter(Yicongh1ZicanlHwoLink)
@Getter(Yicongh1ZicanlHwoLink)
class DeleteFrontierFrame(IApplication):
    ...
    def update(self):
        ...
        ls = self.frame.get(Yicongh1ZicanlHwoLink)
        print "Found ", len(ls), " links."
        count = 0
        for l in ls:
            if not is_valid(l.full_url):
                self.frame.delete(Yicongh1ZicanlHwoLink, l)
                count += 1
...
class Simulation(object):
    ...
    def __init__(self, address, port):
        ...
        objectless_connector = ObjectlessSpacetimeConnection(
            "ResetFrontier_Yicongh1ZicanlHwo".format(CrawlerFrame.app_id),
